...
def migrate_database():
    ...
    try:
        ...
        # ========== schedules 表：实际上课时间迁移 ==========
        if not check_column_exists('schedules', 'actual_started_at'):
            logger.info("🔧 迁移 #11: 给 schedules 表添加 actual_started_at 列")
            db.execute(text("ALTER TABLE schedules ADD COLUMN actual_started_at TIMESTAMP"))
            db.commit()
            logger.info("✅ schedules.actual_started_at 列添加成功")
            migration_count += 1

        if not check_column_exists('schedules', 'actual_ended_at'):
            logger.info("🔧 迁移 #12: 给 schedules 表添加 actual_ended_at 列")
            db.execute(text("ALTER TABLE schedules ADD COLUMN actual_ended_at TIMESTAMP"))
            db.commit()
            logger.info("✅ schedules.actual_ended_at 列添加成功")
            migration_count += 1

        if not check_column_exists('schedules', 'actual_duration_minutes'):
            logger.info("🔧 迁移 #13: 给 schedules 表添加 actual_duration_minutes 列")
            db.execute(text("ALTER TABLE schedules ADD COLUMN actual_duration_minutes INTEGER"))
            db.commit()
            logger.info("✅ schedules.actual_duration_minutes 列添加成功")
            migration_count += 1
...
def verify_migrations():
    ...
    required_columns = {
        ...
        'schedules': ['scheduled_at', 'teacher_id', 'session_id', 'actual_started_at', 'actual_ended_at', 'actual_duration_minutes'],
