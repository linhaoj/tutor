import axios from 'axios'
  ...
  export const studentAPI = {
    getStudents: (): Promise<Student[]> => api.get('/api/students'),
    getStudent: (id: number): Promise<Student> => api.get(`/api/students/${id}`),
    ...
    getStudentStats: (id: number): Promise<any> => api.get(`/api/students/${id}/stats`)
  }

  // 单词相关API
  export const wordAPI = {
    getWords: (wordSet?: string): Promise<Word[]> =>
      api.get('/api/words', { params: { word_set: wordSet } }),
    getWordSets: (): Promise<{ name: string }[]> => api.get('/api/words/sets'),
